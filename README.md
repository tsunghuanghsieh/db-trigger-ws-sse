...
## Frontend
Upon loading, it retrieves the latest count value. It has a button to increment the count in a database table and a push from backend. The only difference between SSE and WS is that SSE uses a EventSource to receive the latest count value, while WS uses a websocket.
